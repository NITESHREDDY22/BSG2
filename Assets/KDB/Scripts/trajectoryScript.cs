...
public class trajectoryScript : MonoBehaviour {
	...
	private Vector3 ballPos,firstTouchPos;					//Position of the ball
  ...
  //      }
        ...
        if ((Input.GetKey (KeyCode.Mouse0) && ballIsClicked == true) && ((ballRB.velocity.x == 0f && ballRB.velocity.y == 0f) || (grabWhileMoving == true))) {	//If player has activated a shot										//when you press down
			...
			ballFingerDiff = firstTouchPos - fingerPos;								//The distance between the finger/cursor and the "ball" is found
			
			shotForce = new Vector2 (ballFingerDiff.x * shootingPowerX, ballFingerDiff.y * shootingPowerY);	//The velocity of the shot is found

			if ((Mathf.Sqrt ((ballFingerDiff.x * ballFingerDiff.x) + (ballFingerDiff.y * ballFingerDiff.y)) > (0.6f))) { //If the distance between the finger/cursor and the "ball" is big enough...
        ...
        if(timeSpend > 5 && ballRB.velocity.magnitude <.1f)
        {
            Debug.Log("Stuck somewhere respawn now"+ ballRB.velocity.magnitude);
            timeSpend = 0;
        }
    }
    float timeSpend = 0;
    ...
    {
        yield return new WaitForSeconds(.625f / 1.5f);//animation duration .625
    ...
    [SerializeField] bool NeedMask = false;
    ...
    private void OnCollisionEnter2D(Collision2D collision)
    {
        ...
        Debug.Log("hits "+collision.collider.name);
    ...
    void HideBallSkin(bool need)
    {
            //GetComponent<SpriteRenderer>().enabled = !need;
        //var em = transform.Find("CFX_Firecomic").GetComponent<ParticleSystem>().emission;
        //em.enabled = !need;
    }
}
