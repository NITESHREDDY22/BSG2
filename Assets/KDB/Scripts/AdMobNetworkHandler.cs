...
public class AdMobNetworkHandler :MonoBehaviour
{
    ...
    public Action<bool,AdType> rewardedInterStitialrequestcallBack = null;
                ...
                InterstitialAd.Load(item.AdID, new AdRequest(),
                    (InterstitialAd ad, LoadAdError loadAdError) =>
                    {
                        ...
                        MobileAdsEventExecutor.ExecuteInUpdate(() =>
                        {
                            ...
                            {
                                HandlePaidEvent(adValue, adUnitId, "interstitial");
                            };
    ...
    private void HandlePaidEvent(AdValue adValue, string adUnitId, string adFormat)
    {
        // Calculate revenue immediately
        double revenue = adValue.Value / 1_000_000.0;
        string currency = adValue.CurrencyCode;
        int precision = (int)adValue.Precision;

        // Move to Main Thread for Analytics SDKs
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            ...
            new Parameter("ad_value_precision", precision.ToString())
            };
                if (GameConstants.GetAllLevelsCompleteStatus() != string.Empty)
                {
                    worldEventName = $"{GameConstants.GetAllLevelsCompleteStatus()}_{worldEventName}";
                }
            ...
            try
            {
                ...
                FB.LogAppEvent("AdImpression", (float)revenue, metadata);
            }
            catch (Exception e) { Debug.LogError($"FB Log Fail: {e.Message}"); }

            // 3. Singular Logging (Adding back what was in your original file)
            /* try
            {
                Singular.SingularAdData data = new Singular.SingularAdData("Admob", currency, revenue);
                Singular.SingularSDK.AdRevenue(data);
            }
            catch (Exception e) { Debug.LogError($"Singular Log Fail: {e.Message}"); } */

            Debug.Log($"<color=green>[Revenue Recorded]</color> {adFormat} | {revenue} {currency}");
        });
    }
    ...
    void OnAdLoadFailed(AdType adType)
    {
        ...
        // Trigger Fallback logic in AdManager
        /* if (adType == AdType.Launch || adType == AdType.Interstital)
        { */
            rewardedInterStitialrequestcallBack?.Invoke(false,adType);
        /* } */
    }

    public void RequestWithManualDelay(AdType adType, float delay)
    {
        StopPreviousCoroutine();
        insterstitalCoroutine = StartCoroutine(RequestDelay(delay, () => {
            AdTestToast.Instance?.Show($"Retry: Cooldown finished. Requesting {adType}...");
            RequestInterstitial(adType);
        }));
    }

    // Check if Launch Ad is actually ready for Splash screen
    public bool LaunchInterstitialState() => adMobLaunchInterstitial != null && adMobLaunchInterstitial.CanShowAd();

    public void SetLaunchId(string newId) { 
        if (keyValuePairs.ContainsKey(AdType.Launch)) keyValuePairs[AdType.Launch].AdID = newId; 
    }
    ...
    public static string LastRewardLoadError = "No_Error_Captured";
            ...
            if (!item.isAdReady)
            {
                if (item.RewardedAd != null)
                {
                    ...
                    item.RewardedAd = null;
                     AdTestToast.Instance?.Show($"AdMob: Requesting Reward {adType}");
                    item.isAdRequested = true;
                ...
                RewardedAd.Load(item.AdID, new AdRequest(),
                   (RewardedAd ad, LoadAdError loadError) =>
                   {
                       if (loadError != null)
                       {
                           MobileAdsEventExecutor.ExecuteInUpdate(() =>
                           {
                            AdTestToast.Instance?.Show($"Reward {adType} FAIL: {loadError.GetMessage()}");
                            LastRewardLoadError = loadError.GetMessage().Replace(" ", "_"); 
                               item.isAdRequested = false;
                               item.isAdReady = false;
                               RequestAgain(adType);
                       ...
                       else if (ad == null)
                       {
                           MobileAdsEventExecutor.ExecuteInUpdate(() =>
                           {
                            AdTestToast.Instance?.Show($"Reward {adType} FAIL: Null Object");
                               item.isAdRequested = false;
                               item.isAdReady = false;
                                RequestAgain(adType);
                       ...
                       AdTestToast.Instance?.Show($"Reward {adType} LOADED Successfully");
                       ...
                       if (!item.isAdReady)
                       {
                           ...
                           {
                                HandlePaidEvent(adValue, adUnitId, "rewarded");
                           };
                       }
                   });
            }
    ...
    public void ShowAdmobRewardedVideo(Action<bool> callBack,AdType adType=AdType.Reward)
    {
        ...
        if (item!=null && item.RewardedAd!= null && item.RewardedAd.CanShowAd())
        {
            ...
            MobileAdsEventExecutor.ExecuteInUpdate(() =>
            {                
                AdTestToast.Instance?.Show($"Displaying Reward {adType}...");
                item.RewardedAd.Show((Reward reward) =>
                {
                    if (reward != null)
                    {
                        Debug.LogError("asdf Admob  Rewarded ad granted a reward: "+adItem.AdID);
                        AdTestToast.Instance?.Show($"REWARD EARNED: {reward.Amount} {reward.Type}");
        ...
        else
        {
            MobileAdsEventExecutor.ExecuteInUpdate(() =>
            {
                AdTestToast.Instance?.Show($"Reward {adType} Not Ready to Show");
                callBack?.Invoke(false);
        ...
        MobileAdsEventExecutor.ExecuteInUpdate(() =>
        {
            if (!item.isAdReady)
            {
                if (item.adMobRewardedInterstitial != null)
                {
                    ...
                    AdTestToast.Instance?.Show($"AdMob: Requesting Rewarded Interstitial");
                    item.isAdRequested = true;
                    //TODO : FireBaseActions(adType == AdType.Reward ? AdContent.AdMobRewardRequested : AdContent.AdMobContinueRewardRequested, AdMode.Requested, SuccessStatus.Success);
                ...
                RewardedInterstitialAd.Load(item.AdID, new AdRequest(),
                   (RewardedInterstitialAd ad, LoadAdError loadError) =>
                   {
                       if (loadError != null)
                       {
                           MobileAdsEventExecutor.ExecuteInUpdate(() =>
                           {
                                AdTestToast.Instance?.Show($"Rew-Inter FAIL: {loadError.GetMessage()}");
                               item.isAdRequested = false;
                               item.isAdReady = false;
                               RequestWithDelay(adDelayTimer, () =>
                               {
                       ...
                       else if (ad == null)
                       {
                           MobileAdsEventExecutor.ExecuteInUpdate(() =>
                           {
                                AdTestToast.Instance?.Show($"Rew-Inter FAIL: Null Object");
                               item.isAdRequested = false;
                               item.isAdReady = false;
                               RequestWithDelay(adDelayTimer, () =>
                               {
                       ...
                       AdTestToast.Instance?.Show($"Rew-Inter LOADED Successfully");
                       if (!item.isAdReady)
                       {
                           item.adMobRewardedInterstitial = ad;
                           item.isAdReady = true;
                           string adUnitId = item.AdID;
                           this.rewardedInterStitialrequestcallBack?.Invoke(true,adType);
                           ...
                           ad.OnAdPaid += (AdValue adValue) =>
                           {
                               HandlePaidEvent(adValue, adUnitId, "rewarded_interstitial");
                           };
    ...
    public void ShowRewardInterstitial(Action<bool> callBack, AdType adType = AdType.RewardedInterStitial)
    {
       ...
       this.rewardedInterStitialcallBack = callBack;
        if (item != null && item.adMobRewardedInterstitial != null && item.adMobRewardedInterstitial.CanShowAd())
        {
            MobileAdsEventExecutor.ExecuteInUpdate(() =>
            {
                AdTestToast.Instance?.Show("Displaying Rewarded Interstitial...");
                item.adMobRewardedInterstitial.Show((Reward reward) =>
                {
                    if (reward != null)
                    {
                        AdTestToast.Instance?.Show("REWARD EARNED (Interstitial)!");
        ...
        else
        {
            MobileAdsEventExecutor.ExecuteInUpdate(() =>
            {
                AdTestToast.Instance?.Show("Rewarded Interstitial Not Ready");
                callBack?.Invoke(false);
    ...
    public void CreateBannerView()
    {
       ...
       // int deviceWidth = Mathf.RoundToInt(width / Screen.dpi * 160); // Convert px to dp if needed
        ...
        {
            HandlePaidEvent(adValue, adUnitId, "banner");
        };
    }
                ...
                AppOpenAd.Load(item.AdID, new AdRequest(),
                        (AppOpenAd ad, LoadAdError loadAdError) =>
                        {
                            ...
                            if (!item.isAdReady)
                            {
                                ...
                                {
                                    HandlePaidEvent(adValue, adUnitId, "appopen");
                                };
                            }
